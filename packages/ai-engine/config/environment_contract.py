...
def enforce_delta_environment(config: Any) -> str:
    ...
    The historical settings file contained production endpoints for testnet.
    This function corrects the effective runtime object and then verifies that
    production and testnet cannot resolve to the same REST endpoint.
    """
    delta = config.delta
    if delta.testnet:
        corrected = replace(
