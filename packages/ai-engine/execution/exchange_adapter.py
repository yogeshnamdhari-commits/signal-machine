...
class ExchangeAdapter:
    ...
    async def _request(
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                if method == "POST":
                    async with session.post(url, params=request_params) as resp:
                        return await self._handle_response(resp, path, attempt)
                if method == "DELETE":
                    async with session.delete(url, params=request_params) as resp:
                        return await self._handle_response(resp, path, attempt)
                if method == "PUT":
                    async with session.put(url, params=request_params) as resp:
                        return await self._handle_response(resp, path, attempt)
    ...
    async def place_order(self, symbol: str, side: OrderSide, order_type: OrderType,
                          quantity: float, price: float = 0.0, stop_price: float = 0.0,
                          time_in_force: TimeInForce = TimeInForce.GTC,
                          reduce_only: bool = False, close_position: bool = False,
                          position_side: PositionSide = PositionSide.BOTH,
                          client_order_id: str = "", working_type: str = "CONTRACT_PRICE",
                          callback_rate: float = 0.0) -> ExchangeOrder:
        params: Dict[str, Any] = {"symbol": symbol, "side": side.value, "type": order_type.value}
        if client_order_id:
        ...
        if order_type in (OrderType.STOP_MARKET, OrderType.TAKE_PROFIT_MARKET, OrderType.TRAILING_STOP_MARKET):
            if stop_price > 0:
    ...
    async def cancel_order(self, symbol: str, order_id: int = 0, client_order_id: str = "") -> ExchangeOrder:
    ...
    async def get_order(self, symbol: str, order_id: int = 0, client_order_id: str = "") -> ExchangeOrder:
        params: Dict[str, Any] = {"symbol": symbol}
        if order_id:
            params["orderId"] = order_id
        if client_order_id:
    ...
    async def get_all_orders(self, symbol: str, limit: int = 500) -> List[ExchangeOrder]:
        result = await self._request("GET", "/fapi/v1/allOrders",
        ...
        for p in result:
            ...
            if qty != 0:
                positions.append(ExchangePosition(
                    ...
                    quantity=abs(qty), entry_price=float(p.get("entryPrice", 0)),
    ...
    def _get_symbol_filters(self, symbol: str) -> Dict:
        ...
        if not filters:
            raise ExchangeError(f"Symbol filters unavailable for {symbol}; refusing to format a live order")
        return filters
    ...
    def _format_price(self, symbol: str, price: float) -> str:
        ...
        if not tick or tick <= 0:
            raise ExchangeError(f"Invalid exchange tick size for {symbol}; refusing live order")
    ...
    def _format_quantity(self, symbol: str, qty: float) -> str:
        ...
        if not step or step <= 0:
            raise ExchangeError(f"Invalid exchange step size for {symbol}; refusing live order")
            ...
            for f in s.get("filters", []):
                if f["filterType"] == "PRICE_FILTER":
                    ...
                    if notional is not None:
                        filters["min_notional"] = float(notional)
            required = ("tick_size", "step_size", "min_qty", "max_qty")
            if not all(k in filters for k in required):
                logger.error("Incomplete exchange filters for {}; refusing cache", s["symbol"])
                continue
