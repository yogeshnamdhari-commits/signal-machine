...
def test_record_trade_includes_funding_in_net_pnl(tmp_path):
    ...
    row = db.query(
        "SELECT gross_pnl, net_pnl, funding, slippage, costs_observed, outcome "
...
def test_record_trade_persists_realized_pnl_decomposition(tmp_path):
    ...
    row = db.query(
        "SELECT gross_pnl, fees, funding, slippage, realized_pnl_raw, realized_pnl_rounded, net_pnl "
        "FROM forward_trades WHERE id=?",
        (trade_id,),
    )[0]
...
def test_record_trade_marks_missing_cost_inputs_as_unobserved(tmp_path):
    db = ForwardTestDB(str(tmp_path / "forward_test.db"))
    ...
    row = db.query("SELECT costs_observed FROM forward_trades WHERE id=?", (trade_id,))[0]
    assert row["costs_observed"] == 0
