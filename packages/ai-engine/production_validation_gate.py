"""Production repository gate used immediately before dev is promoted to main."""
...
def ticker_handler_emits_trade(source: str) -> bool:
    tree = ast.parse(source)
    for class_node in tree.body:
        if not isinstance(class_node, ast.ClassDef) or class_node.name != "BinanceWebSocket":
            continue
        for method in class_node.body:
            if not isinstance(method, (ast.FunctionDef, ast.AsyncFunctionDef)) or method.name != "_on_ticker_arr":
                continue
            for node in ast.walk(method):
                if not isinstance(node, ast.Call):
                    continue
                func = node.func
                if not isinstance(func, ast.Attribute) or func.attr != "_callback" or not node.args:
                    continue
                first = node.args[0]
                if isinstance(first, ast.Constant) and first.value == "trade":
                    return True
    return False
...
def main() -> None:
    ...
    failures.extend(f"missing required contract module: {path}" for path in required if not path.exists())
    ...
    if adapter.exists():
        ...
        try:
            if ticker_handler_emits_trade(text):
                failures.append("ticker24h handler can emit trade callbacks")
        except SyntaxError as exc:
            failures.append(f"Binance adapter parse failure during ticker audit: {exc}")
    ...
    print("Production validation gate passed.")
