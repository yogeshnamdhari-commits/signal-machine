...
Simulates trades using historical data with slippage, fees, funding and position management.
"""
...
class Trade:
    ...
    def net_pnl(self) -> float:
        """Net economic PnL includes every modeled cost."""
        return self.pnl - self.fees - self.slippage - self.funding
...
class BacktestEngine:
    """Causal, deterministic event-driven backtesting engine."""
    ...
    async def initialize(self) -> None:
        ...
        self._rng = np.random.default_rng(self.config.random_seed)
        logger.info(
            "BacktestEngine ready — capital: ${:,.0f}, leverage: {}x, seed={}",
    ...
    async def run(
        ...
        if self.config.require_funding_column and "funding_rate" not in data.columns:
            raise ValueError("Backtest configured to require historical funding_rate, but the data has none")

        logger.info(
            "Running backtest: {} — {} bars from {} to {}",
            symbol,
            len(data),
        ...
        for i in range(50, len(data)):
            ...
            await self._check_exits(symbol, high, low, price, bar_time, row)

            if self.config.trailing_stop:
                for pos in list(state.positions):
                    if pos.symbol == symbol and pos in state.positions:
                        await self._update_trailing_stop(pos, high, low, bar_time, row)
            ...
            # Execute signals only after the configured latency has elapsed.
            scheduled = pending.pop(i, None)
            if scheduled and self._can_open_position(symbol):
                await self._open_position(symbol, scheduled, price, bar_time, row)

            # Strategy receives a strictly causal snapshot. Future rows are inaccessible.
            try:
                causal = data.iloc[: i + 1].copy(deep=True)
                signal = signal_func(causal, len(causal) - 1)
            except Exception as exc:
                logger.debug("Signal error at bar {}: {}", i, exc)
                signal = None

            if signal and self._can_open_position(symbol):
                execution_index = i + latency
                if execution_index < len(data):
                    pending[execution_index] = dict(signal)

        last_price = float(data["close"].iloc[-1])
        last_time = data["open_time"].iloc[-1]
        last_row = data.iloc[-1]
        for pos in list(state.positions):
            await self._close_position(pos, last_price, last_time, "backtest_end", last_row)

        # After all positions are closed, capital is the authoritative final equity.
        state.equity_curve.append(state.capital)
        result = self._build_result(symbol, data)
        logger.info(
            "Backtest complete: {} — trades={}, win_rate={:.1f}%, pnl=${:,.2f}",
            symbol,
            result.total_trades,
            result.win_rate,
            result.net_pnl,
    ...
    async def _open_position(
        ...
        is_adverse = self._rng.random() < self.config.adverse_fill_prob
        slippage = base_slippage * (self.config.adverse_fill_mult if is_adverse else 1.0)

        entry_price = price + spread_cost / 2 + slippage if side == Side.LONG else price - spread_cost / 2 - slippage

        actual_size = size
        partial_fills = 0
        if self.config.partial_fill_rate > 0 and self._rng.random() < self.config.partial_fill_rate:
            actual_size = size * float(self._rng.uniform(0.7, 0.95))
            partial_fills = 1
        ...
        trailing_stop = None
        if self.config.trailing_stop:
            trailing_stop = entry_price * (1 - self.config.trailing_stop_pct) if side == Side.LONG else entry_price * (1 + self.config.trailing_stop_pct)
    ...
    async def _close_position(
        ...
        base_slippage = max(exit_price * self.config.slippage_pct, exit_price * self.config.min_slippage_bps / 10_000)
        is_adverse = self._rng.random() < self.config.adverse_fill_prob
        slippage = base_slippage * (self.config.adverse_fill_mult if is_adverse else 1.0)
        ...
        funding_rate = self.config.funding_rate_pct
        if row is not None and "funding_rate" in row.index and pd.notna(row["funding_rate"]):
            funding_rate = float(row["funding_rate"])
        bars_per_funding = 96
        funding_periods = pos.bars_held / bars_per_funding
        funding = pos.entry_price * pos.size * funding_rate * funding_periods
    ...
    async def _check_exits(
        ...
        for pos in list(state.positions):
            ...
            hit_tp = False
            if pos.side == Side.LONG:
                hit_sl = bool(pos.stop_loss and low <= pos.stop_loss)
                hit_tp = bool(pos.take_profit and high >= pos.take_profit)
            else:
                hit_sl = bool(pos.stop_loss and high >= pos.stop_loss)
                hit_tp = bool(pos.take_profit and low <= pos.take_profit)

            if hit_sl and hit_tp:
                if self.config.same_bar_exit_policy == "tp_first":
                    await self._close_position(pos, pos.take_profit, bar_time, "take_profit_same_bar", row)
                else:
                    await self._close_position(pos, pos.stop_loss, bar_time, "stop_loss_same_bar", row)
            elif hit_sl:
                await self._close_position(pos, pos.stop_loss, bar_time, "stop_loss", row)
            elif hit_tp:
                await self._close_position(pos, pos.take_profit, bar_time, "take_profit", row)
    ...
    async def _update_trailing_stop(
        ...
        if pos.side == Side.LONG:
            ...
            if low <= pos.trailing_stop_price:
                await self._close_position(pos, pos.trailing_stop_price, bar_time, "trailing_stop", row)
        else:
    ...
    def _can_open_position(self, symbol: str) -> bool:
        ...
        if any(p.symbol == symbol for p in state.positions):
            return False
        return state.capital > 0

    def _calculate_equity(self, current_price: float) -> float:
        state = self._state
        equity = state.capital
        for pos in state.positions:
            unrealized = (current_price - pos.entry_price) * pos.size if pos.side == Side.LONG else (pos.entry_price - current_price) * pos.size
    ...
    def _build_result(self, symbol: str, data: pd.DataFrame) -> "BacktestResult":
        ...
        final_equity = state.capital if not state.positions else self._calculate_equity(float(data["close"].iloc[-1]))
