...
class ForwardTestDB:
    ...
    def _init_db(self) -> None:
        ...
        for index in (
            "CREATE INDEX IF NOT EXISTS idx_fs_ts ON forward_signals(timestamp)",
            "CREATE INDEX IF NOT EXISTS idx_fs_regime ON forward_signals(regime)",
            "CREATE INDEX IF NOT EXISTS idx_fs_session ON forward_signals(session)",
            "CREATE INDEX IF NOT EXISTS idx_fs_conf ON forward_signals(confidence_100)",
            "CREATE INDEX IF NOT EXISTS idx_ft_ts ON forward_trades(timestamp)",
            "CREATE INDEX IF NOT EXISTS idx_ft_regime ON forward_trades(regime)",
            "CREATE INDEX IF NOT EXISTS idx_ft_session ON forward_trades(session)",
            "CREATE INDEX IF NOT EXISTS idx_ft_outcome ON forward_trades(outcome)",
            "CREATE INDEX IF NOT EXISTS idx_ft_symbol ON forward_trades(symbol)",
        ):
            db.execute(index)
    ...
    def record_signal(self, sig: Dict[str, Any]) -> int:
        ...
        """, (
            sig.get("timestamp", time.time()), sig.get("symbol", ""), sig.get("side", ""),
            sig.get("confidence_100", 0), sig.get("institutional_score", 0),
            sig.get("regime", ""), sig.get("session", ""), sig.get("entry_price", 0),
            sig.get("stop_loss", 0), sig.get("take_profit", 0), sig.get("risk_reward", 0),
            sig.get("delta", 0), sig.get("cvd", 0), sig.get("oi_delta", 0),
            sig.get("funding_rate", 0), sig.get("sweep_score", 0), sig.get("mss_score", 0),
            sig.get("fvg_score", 0), sig.get("entry_reason", ""), sig.get("signal_status", "pending"),
            sig.get("mtf_alignment", 0), sig.get("checklist_score", 0),
            sig.get("regime_confidence", 0), sig.get("volatility_score", 0),
            1 if sig.get("quiet_market_blocked") else 0, json.dumps(sig.get("metadata", {})),
        ))
        db.commit()
    ...
    def record_trade(self, trade: Dict[str, Any]) -> int:
        ...
        """, (
            ...
            trade.get("confidence_100", 0), trade.get("institutional_score", 0),
            trade.get("sweep_score", 0), trade.get("mss_score", 0), trade.get("fvg_score", 0),
            trade.get("delta", 0), trade.get("cvd", 0), trade.get("oi_delta", 0),
            trade.get("funding_rate", 0), outcome,
        ))
        db.commit()
        row_id = cursor.lastrowid
        db.close()
    ...
    def get_collection_status(self) -> Dict:
        ...
        regimes = dict(db.execute("SELECT regime, COUNT(*) FROM forward_signals GROUP BY regime").fetchall())
        sessions = dict(db.execute("SELECT session, COUNT(*) FROM forward_signals GROUP BY session").fetchall())
