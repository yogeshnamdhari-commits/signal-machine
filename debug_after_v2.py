#!/usr/bin/env python3
"""Quick debug: count signals for new AFTER strategy on 5 symbols."""
...
def compute_adx(highs, lows, closes, period=14):
    ...
    for i in range(period + 1, n):
        atr_s[i] = atr_s[i-1] - atr_s[i-1]/period + tr[i]; pdm_s[i] = pdm_s[i-1] - pdm_s[i-1]/period + pdm[i]; ndm_s[i] = ndm_s[i-1] - ndm_s[i-1]/period + ndm[i]
    for i in range(period, n):
        if atr_s[i] > 0: plus_di[i] = 100.0 * pdm_s[i] / atr_s[i]; minus_di[i] = 100.0 * ndm_s[i] / atr_s[i]
...
def main():
    ...
    test_syms = ['BTCUSDT', 'ETHUSDT', 'FILUSDT', 'DOGEUSDT', 'SOLUSDT']
    ...
    for sym in test_syms:
        ...
        # Count BEFORE signals (same logic)
        before_signals = 0
        after_signals = 0
        after_detail = {'chop': 0, 'atr': 0, 'regime': 0, 'trend': 0, 'pullback': 0,
                       'candle': 0, 'volume': 0, 'confidence': 0, 'rr': 0, 'pass': 0}
        ...
        for i in range(220, n):
            ...
            # BEFORE confidence
            conf_before = 100 * 0.15 + score * 0.25 + 100 * 0.25 + 90 * 0.20 + 50 * 0.15
            if conf_before < 90.0: continue
            
            before_signals += 1
            
            # AFTER: same binary gates passed, now check AFTER-specific confidence
            latest_adx = adx_arr[i]
            rvol = rvol_arr[i] if not np.isnan(rvol_arr[i]) else 0
            
            atr_pct = atr_arr[i] / closes[i] * 100 if closes[i] > 0 else 0
            if atr_pct < 0.05: after_detail['atr'] += 1; continue
            ...
            chop_score = min(100, ema_range_pct / 0.5 * 100)
            ...
            conf_after = (
                100 * 0.10 + score * 0.20 + 100 * 0.15 + candle_score * 0.10 +
                min(100, rvol / 2.0 * 100) * 0.10 +
                min(100, latest_adx / 50 * 100) * 0.15 +
                min(100, rvol / 3.0 * 100) * 0.10 +
                min(100, ema200_score) * 0.10
            )
            if conf_after < 90.0:
                after_detail['confidence'] += 1
                if after_detail['confidence'] <= 5:
                    print(f"  CONF MISS: conf_after={conf_after:.2f} score={score} adx={latest_adx:.1f} rvol={rvol:.2f} candle={candle_score} ema200_s={ema200_score:.0f}")
                continue
            
            after_signals += 1
        
        print(f"{sym}: BEFORE={before_signals} signals, AFTER={after_signals} signals")
        for k, v in sorted(after_detail.items(), key=lambda x: -x[1]):
            if v > 0: print(f"  {k}: {v}")
    conn.close()
